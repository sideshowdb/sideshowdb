...
fn buildTests(
    ...
    const write_behind_ref_test_mod = b.createModule(.{
        .root_source_file = b.path("tests/write_behind_ref_store_test.zig"),
        .target = target,
        .optimize = optimize,
        .imports = &.{
            .{ .name = "sideshowdb", .module = core_mod },
    ...
    const write_behind_ref_tests = b.addTest(.{ .root_module = write_behind_ref_test_mod });
    const run_write_behind_ref_tests = b.addRunArtifact(write_behind_ref_tests);
    ...
    test_step.dependOn(&run_write_behind_ref_tests.step);
