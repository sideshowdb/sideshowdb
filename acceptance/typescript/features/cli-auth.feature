...
Feature: CLI auth subcommands
  ...
  # EARS:
  # - When `sideshow auth status` is invoked and no hosts are configured, the CLI shall exit with code 0 and print "No authenticated hosts." (CLIA-010).
  # - When `sideshow gh auth login --with-token` is invoked, the CLI shall persist the token under hosts.toml and exit 0 (CLIA-030).
  ...
  # - When `sideshow --json auth status` runs against a configured host, the CLI shall emit a JSON object with a token_preview but no full token (CLIA-012).
  # - When `sideshow auth logout --host <h>` is invoked for a known host, the CLI shall remove the entry and exit 0 (CLIA-020).
  # - When `sideshow auth logout --host <h>` is invoked for an unknown host, the CLI shall exit 1 with "not logged in" (CLIA-021).
  # - When `sideshow --refstore github` is invoked without `--repo`, the CLI shall exit 1 before any HTTP request (CLIA-050).
